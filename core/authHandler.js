...
var crypto = require('crypto') // Updated to use crypto for secure hashing
...
module.exports.resetPw = function (req, res) {
	if (req.query.login) {
		db.User.find({
			...
			if (user) {
				// Use SHA-256 for secure token hashing
				const token = crypto.createHash('sha256').update(req.query.login).digest('hex');
	...
	if (req.body.password && req.body.cpassword && req.body.login && req.body.token) {
		if (req.body.password == req.body.cpassword) {
			db.User.find({
				...
				if (user) {
					// Use SHA-256 for secure token hashing
					const token = crypto.createHash('sha256').update(req.body.login).digest('hex');
